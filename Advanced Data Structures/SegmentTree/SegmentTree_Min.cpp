...
void initialize(int node, int start, int end, int* M, int* A)
{
  ...
  else
  {
      ...
      M[node] = min(M[node*2], M[2*node + 1]);
  }
} 

int getMin(int node, int start, int end, int rstart, int rend, int* M)
{
   if(start > end)return 0;
	...
	int a = getMin(2*node, start, (start+end)/2, rstart, rend, M);
	return min(a, getMin(2*node+1, (start+end)/2 + 1, end, rstart, rend, M));
}
...
int main(int argc, char const *argv[])
{
	...
	cout << getMin(1, 1, asize, 3, 8, M) << endl;
